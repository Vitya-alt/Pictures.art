...
var filter = function filter() {
  ...
  function eventClick(btn, mark) {
    btn.addEventListener('click', function () {
      typeFilter(mark);
    });
  }

  eventClick(btnAll, markAll);
  eventClick(btnGirl, markGirl);
  eventClick(btnLovers, markLovers);
  eventClick(btnChef, markChef);
  eventClick(btnGuy, markGuy);
...
var forms = function forms() {
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var name = a[0].substring(0, 6) + dots + a[1];
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  var btn = document.querySelector(trigger); //*Getting cards from layout
